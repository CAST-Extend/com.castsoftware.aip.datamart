...
def load_snapshots(extract_directory):
  snapshots = set()
  ifile = extract_directory+"\\DIM_SNAPSHOTS.csv"
  skip = True
...
def load_rules(extract_directory):
  rules = set()
  ifile = extract_directory+"\\DIM_RULES.csv"
  skip = True
  with open(ifile) as csv_file:  
    csv_reader = csv.reader(csv_file, delimiter=DELIMITER)
    for row in csv_reader:
...
def transform(mode, extract_directory, transform_directory, table_name, nb_primary_columns):
  ...
  with open(ifile) as csv_file:
    ...
    for row in csv_reader:
      ...
      try: 
        previous_snapshot_id_position = row.index('previous_snapshot_id') 
        if not row[previous_snapshot_id] in snapshots:
          print("\tSKIP unknown snapshot: " + row[previous_snapshot_id_position])
          SKIPS=True
          continue
      except:
        pass
...
if __name__ == "__main__":
  ...
  rules = load_rules(args.extract_directory)
  snapshots = load_snapshots(args.extract_directory)
  ...
  if SKIPS:
    print("========================================================================")
    print("WARNING:")
    print("There are some skipped snapshots/rules")
    print("Some snapshots may have been started during the Datamart extraction")
    print("Re-run Datamart to get the most recent data")
    print("========================================================================")
