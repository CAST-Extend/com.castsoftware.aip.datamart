...
MAX(CASE WHEN m.business_criterion_name = 'ISO-5055-Index'::text THEN m.omg_technical_debt/(8*60) ELSE NULL::integer END) AS iso_index_effort_days,
MAX(CASE WHEN m.business_criterion_name = 'ISO-5055-Security'::text THEN m.omg_technical_debt/(8*60) ELSE NULL::integer END) AS iso_security_effort_days,
MAX(CASE WHEN m.business_criterion_name = 'ISO-5055-Reliability'::text THEN m.omg_technical_debt/(8*60) ELSE NULL::integer END) AS iso_reliability_effort_days,
MAX(CASE WHEN m.business_criterion_name = 'ISO-5055-Performance-Efficiency'::text THEN m.omg_technical_debt/(8*60) ELSE NULL::integer END) AS iso_performance_effort_days,
MAX(CASE WHEN m.business_criterion_name = 'ISO-5055-Maintainability'::text THEN m.omg_technical_debt/(8*60) ELSE NULL::integer END) AS iso_maintainability_effort_days,
