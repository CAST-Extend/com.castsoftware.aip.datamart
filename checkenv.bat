...
IF EXIST "%INSTALLATION_FOLDER%\thirdparty\curl-8.18\bin" SET PATH=%INSTALLATION_FOLDER%\thirdparty\curl-8.18\bin;%PATH%
...
IF NOT DEFINED EXTRACT_ZERO_WEIGHT (SET EXTRACT_ZERO_WEIGHT=OFF)
IF NOT [%EXTRACT_ZERO_WEIGHT%] == [ON] IF NOT [%EXTRACT_ZERO_WEIGHT%] == [OFF] (echo ERROR: Invalid EXTRACT_ZERO_WEIGHT value %EXTRACT_ZERO_WEIGHT%, expecting ON or OFF & EXIT /b 1)
